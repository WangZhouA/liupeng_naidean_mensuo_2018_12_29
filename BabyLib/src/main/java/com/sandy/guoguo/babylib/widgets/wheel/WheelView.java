...
import com.sandy.guoguo.babylib.widgets.wheel.listens.OnWheelClickedListener;
...
//	private int itemHeight = 30;
    ...
    /** 背景 图片资源ID */
//	private final int BACKGROUNDRESID = R.drawable.height_weight_dialog_item_bg;
    /**
     * 选中 图片资源ID
     */
    private final int CHECKDRAWABLERESID = R.drawable.ic_massage_item_check;
    /**
     * 是否显示顶部和底部的阴影
     */
    private final boolean ISSHOWTOPANDBOTTOMSHADOWS = false;
    /**
     * 顶部和底部的阴影的颜色值
     */
    private final int[] SHADOWS_COLORS = new int[]{0xFFFFFFFF, 0x00FFFFFF, 0x00FFFFFF};
    ...
    private void initResourcesIfNecessary() {
        if (centerDrawable == null) {
            centerDrawable = getContext().getResources().getDrawable(CHECKDRAWABLERESID);
        }
...
//		setBackgroundResource(BACKGROUNDRESID);
    ...
    private int getItemHeight() {
        return layoutHeight / visibleItems;
    }
...
//		setMeasuredDimension(width+4*PADDING, height);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (ISSHOWTOPANDBOTTOMSHADOWS) {
            drawShadows(canvas);
        }
    }

    /**
     * 画头部和底部的阴影 Draws shadows on top and bottom of control
     *
...
//		canvas.translate(PADDING, -top + scrollingOffset);
    ...
    private boolean addViewItem(int index, boolean first) {
        ...
        if (view != null) {
            ...
            v.setHeight(getItemHeight());
            TextPaint tp = v.getPaint();
            if (index == currentItem) {
                v.setTextColor(Color.parseColor("#FF5676b8"));

                v.setTextSize(TypedValue.COMPLEX_UNIT_PX, getContext().getResources().getDimension(R.dimen.font_16));
                ...
                if (index == currentItem - 1 || index == currentItem + 1) {
                    v.setTextColor(Color.parseColor("#BF5676b8"));
                    v.setTextSize(TypedValue.COMPLEX_UNIT_PX, getContext().getResources().getDimension(R.dimen.font_14));
                } else if (index == currentItem - 2 || index == currentItem + 2) {
                    v.setTextColor(Color.parseColor("#7F5676b8"));
                    v.setTextSize(TypedValue.COMPLEX_UNIT_PX, getContext().getResources().getDimension(R.dimen.font_12));
                }
