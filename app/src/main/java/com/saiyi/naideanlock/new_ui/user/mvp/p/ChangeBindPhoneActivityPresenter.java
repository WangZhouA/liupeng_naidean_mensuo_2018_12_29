...
public class ChangeBindPhoneActivityPresenter extends BasePresenter<ChangeBindPhoneActivityView> {
    ...
    public void checkPhone(Map map) {
        if (model != null) {
            view.showLoading();
            model.checkPhone(CHECK_PHONE, map, this);
        }
    }
    public void updatePhone(Map map) {
        if (model != null) {
            view.showLoading();
            model.updatePhone(UPDATE_PHONE, map, this);
        }
    }
    public void getCheckCode(Map map) {
        if (model != null) {
            view.showLoading();
            model.getCheckCode(REQ_GET_CHECK_CODE, map, this);
        }
    }
